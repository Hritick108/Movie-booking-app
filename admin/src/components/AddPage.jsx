...
const AddPage = () => {
  ...
  // file helpers that renders the image uploaded by the user.
  ...
  // slots helpers add remove or update the slot for timings for a movie choose date time and am/pm for that movie
  function addSlot() {
    setSlots((s) => [
      ...s,
  ...
  // Restes the fields once data gets submited to the server.
  function resetForm() {
    setMovieName("");
    setCategories([]);
    setPoster(null);
  ...
  // this funciton validates all the fileds are validated or not with correct details.
  function validate() {
  ...
  // this function will create the movie
  async function handleSubmit(e) {
    ...
    // if movieType === latestTrailer then it will create a latestTrailersObj
    if (movieType === "latestTrailers") {
  ...
  return (
    <div className={addMoviePageStyles.uploadContainer}>
      ...
      <div className={addMoviePageStyles.uploaderGrid}>
        {items && items.length ? (
          items.map((it, idx) => (
            <div key={idx} className={addMoviePageStyles.uploadContainer}>
              <img
                src={it.preview}
                alt="preview"
...
// for latesTrailer we NameUpLoader
