...
const Navbar = () => {
  ...
  }, [open, close]); // this function hels in closing the menu for mobile when click on escp button
  ...
  return (
    <div className={styles4.navbar}>
      <div className={styles4.navbar.contianer}></div>
      <div className={styles4.navbarFlex}>
        ...
        <div className={styles4.logoContainer}>
          ...
          <span className={styles4.logoText}> CINNEVERSE</span>
        </div>

        {/* Desktop Links (unchanged) Look */}
        <div className={styles4.desktopNav}>
          <NavItem to="/" Icon={Film} label="ADD MOVIES" end />
          <NavItem to="/listmovies" Icon={List} label="LIST MOVIES" end />
          <NavItem to="/dashboard" Icon={Calendar} label="DASHBOARD" end />
          <NavItem to="/bookings" Icon={Ticket} label="BOOKINGS" end />
      ...
      <div
        className={`${styles4.mobileMenuContainer} ${
          open ? "pointer-events-none" : "pointer-events-none"
        }`}
      >
        <div
        ...
        <aside
          ...
          className={`${styles4.mobileMenuPanel} ${
            open ? "translate-" : "translate-x-full"
          } `}
          role="dialog"
        >
          <div className={styles4.mobileMenuPanelHeader}>
            <div className={styles4.logoContainer}>
              <div className=" flex items-center justify-center w-10 h-10 bg-red-600 rounded-full transfrom rotate-12">
                <Film className="w-6 h-6 text-whtie transfrom-rotate-12" />
              </div>
              <span className="font-['Impact'] text-xl text-whtie tracking-wider">
                CINEVERSE
              </span>
            </div>
          ...
          <div className={styles4.mobileMenuPanelFooter} >
            <p className={styles4.mobileMenuFooterText} >
              &copy; {new Date().getFullYear()} CINERVERSE
          </p>
          </div>
        </aside>
      </div>
