...
const LoginPage = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prevState) => ({
      ...prevState,
      [name]: value, // [] tells the javascript that use the information stored in name varialble as a key if spelled name: value it will creatte a string name
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Raw Data recived", res);

      const data = res.data;
      if (data && data.success) {
        toast.success(data.message || "Login successfull Redirecting...");
          
        console.log("Before storing th key to the localstorage");
        if (data.token) {
          localStorage.setItem("token", data.token);
        }
        console.log("After storing th key to the localstorage", data.token);
          ...
          <div className={loginStyles.cardContent}>
            <div className={loginStyles.headerContainer}>
              ...
              <p className={loginStyles.headerSubtitle}>
                Enter you credentials to continue the experience
              </p>
            </div>
...
// Remarks : customCSS is not applied properly in this component here
