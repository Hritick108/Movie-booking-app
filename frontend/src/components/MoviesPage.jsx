...
const MoviesPage = () => {
  const [acitveCategory, setActiveCategory] = useState("all");
  ...
  const filteredMovies = React.useMemo(() => {
    if(acitveCategory === "all" ) return movies;
    return movies.filter(
      (m) =>
        String(m.category || "").toLowerCase()  === String(acitveCategory || "").toLowerCase()
    );
  },[movies,acitveCategory]);
           ...
           {loading ? (
            ...
            {filteredMovies.length === 0 && (
              <div className={moviesPageStyles.emptyState}>
                NO MOVIES FOUNG IN THIS CATEGORY
              </div>
            )}
          </div>
