...
export default function SeatSelectorPage() {
  ...
  useEffect(() => {
    ...
    const fetchBooked = async () => {
      ...
      // using token it will seat the booked  seats as unavilble to book again
      try {
      ...
      // fallback: /api/bookings/occupied this funciton will show yout occupied seats  
      try {
        const token = getAuthToken();
        const headers = token ? { Authorization: `Bearer ${token}` } : {};
  ...
  const reclinerPaise =
    reclinerRupee !== null
      ...
      // creats a booking to server and opens a stripe payment gateway
