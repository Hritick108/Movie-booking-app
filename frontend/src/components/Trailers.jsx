...
const mapMovieToTrailerItem = (movie) => {
  ...
  // if the pic is present then it is ok else it will show the placeholder  thumbnail
  if (firstDirector) {
    credits["Director"] = {
...
const Trailers = () => {
  ...
  const [error, setError] = useState(null);
    ...
    async function load() {
      ...
      if(!res.ok) throw new Error(`HTTP ${res.status} `);
        ...
        console.error("Failes to load trailers: ",err);
  ...
  // This funciton helps in to show the selected trailer
  ...
  // This helps in togglin the player
  const togglePlay = () => {
    setIsPlaying((s) => !s);
  };
  ...
  }; // this checks the url and verify it for youtube
  ...
  if(loading) {
    return (
      <div className={trailersStyles.container} >
        <div className=" py-12 text-center test-gray-300" >
          Loading Trialers...
        </div>
      </div>
    );
  ...
  if(error) {
    return (
      <div className={trailersStyles.container} >
        <div className=" py-12 text-center test-red-300" >
          {error}
        </div>
      </div>
    );
          ...
          <div className={trailersStyles.leftSide}>
            <div className={trailersStyles.leftCard}>
              ...
              <div className={trailersStyles.trendingSection}>
                <h3 className={trailersStyles.trendingTitle}>Now Trendinhg</h3>
